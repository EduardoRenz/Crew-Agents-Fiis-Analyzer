...
class FIIReportScraper:
                ...
                if content.startswith(b'%PDF'):
                    ...
                    try:
                        ...
                        if '<html' in text_content or '<!DOCTYPE' in text_content:
                            pdf_match = re.search(r'href=[\'"]([^\'"]*\.pdf)[\'"]', text_content)
                            if pdf_match:
                                pdf_url = pdf_match.group(1)
                                print(f"Encontrado link direto para PDF: {pdf_url}")
                                return self.download_pdf(pdf_url, save_path)
                    except Exception as decode_error:
                        print(f"Erro ao decodificar conteúdo: {str(decode_error)}")
                    return False
            else:
    ...
    def get_report_links(self, fii_code: str) -> List[Tuple[str, str]]:
        ...
        try:
            relatorios_link = self.driver.find_element(By.XPATH, "//a[normalize-space(text())='Relatórios, Relatório Gerencial']")
